...
// main_screen/pages/settings/edit_financial_info_page.dart (UPDATED)
// 경로: lib/main_screen/pages/settings/edit_financial_info_page.dart
...
class EditFinancialInfoPage extends StatefulWidget {
  const EditFinancialInfoPage({Key? key}) : super(key: key);

  @override
  _EditFinancialInfoPageState createState() => _EditFinancialInfoPageState();
}

class _EditFinancialInfoPageState extends State<EditFinancialInfoPage> {
  ...
  Future<void> _loadInitialData() async {
    ...
    if (userId != null) {
      ...
      if (user != null && user.userAssets != null && mounted) {
        setState(() {
          _initializeUnitControllers(user.userAssets!['annualIncome'], _incomeEokController, _incomeCheonController, _incomeBaekController, _incomeSipController);
          _initializeUnitControllers(user.userAssets!['totalAssets'], _assetEokController, _assetCheonController, _assetBaekController, _assetSipController);
          _realEstateValue = user.userAssets!['realEstateValue'];
          _carValue = user.userAssets!['carValue'];
          _debt = user.userAssets!['debt'];
          _descriptionController.text = user.userAssets!['financialDescription'] ?? '';
        });
      }
    }
  }
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      await authService.updateUserFinancialProfile(userId, dataToUpdate);
      if (mounted) {
